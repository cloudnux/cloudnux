...
function calculateNextRun(job: ScheduledJob, preserveNaturalTiming: boolean = true): Date {
    ...
    if (job.cronExpression) {
        ...
        if (!result.isValid) {
            //BREAKPOINT: LOGGING
            //consola.warn(`${chalk.yellow('⚠️  Invalid cron expression for')} ${chalk.green(job.name)}: ${job.cronExpression}`);
            //consola.warn(`${chalk.yellow('Using fallback timing:')} ${result.nextRun}`);
        }
...
function calculateNextRunFromLastRun(job: ScheduledJob, lastRun: Date, config: SchedulerConfig): Date {
    const now = new Date();
...
export const schedulerPlugin: FastifyPluginCallback<SchedulerPluginOptions> = async (
    ...
    if (options.jobs) {
        for (const jobDef of options.jobs) {
            ...
            if (expressionType === "unknown") {
                //BREAKPOINT: LOGGING
                //consola.error(`${logSymbols.error} ${chalk.red('Invalid cron expression for job:')} ${jobDef.name}`);
            ...
            if (config.cron.logCronDetails) {
                ...
                //BREAKPOINT: LOGGING
                //consola.info(`${chalk.blue('📅 Job')} ${chalk.green(jobDef.name)}: ${result.description} - Next: ${chalk.cyan(result.nextRun.toLocaleString())}`);
                ...
                //BREAKPOINT: LOGGING
                //consola.info(`${chalk.blue('   Upcoming:')} ${upcoming.map(d => d.toLocaleTimeString()).join(', ')}`);
            }
    ...
    async function initializeScheduler(): Promise<void> {
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('Scheduler initialized:')} ${Object.keys(schedulers).length} jobs loaded`);
        } catch (error: any) {
            //BREAKPOINT: LOGGING
            //consola.error(`${logSymbols.error} ${chalk.red('Failed to initialize scheduler:')} ${error.message}`);
        }
    }
    ...
    function scheduleJob(scheduler: SchedulerService): void {
        ...
        if (timeUntilNextRun <= 0) {
            ...
            //BREAKPOINT: LOGGING
            //consola.info(`${chalk.blue('📅 Scheduled job')} ${chalk.green(scheduler.job.name)} to run in ${chalk.cyan(Math.round(timeUntilNextRun / 1000))}s`);
        }
    }
    ...
    async function executeJob(scheduler: SchedulerService): Promise<void> {
        ...
        if (scheduler.job.maxRuns && scheduler.job.runCount >= scheduler.job.maxRuns) {
            //BREAKPOINT: LOGGING
            //consola.info(`${chalk.yellow('⏹️  Job')} ${chalk.green(scheduler.job.name)} reached max runs (${scheduler.job.maxRuns})`);
            return;
        }

        if (runningExecutions >= config.execution.maxConcurrent) {
            //BREAKPOINT: LOGGING
            //consola.warn(`${chalk.yellow('⚠️  Delaying job')} ${chalk.green(scheduler.job.name)} - max concurrent limit reached`);
        ...
        //BREAKPOINT: LOGGING
        //consola.info(`${chalk.blue('🚀 Executing job')} ${chalk.green(scheduler.job.name)} (${execution.id})`);
        ...
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('Job completed')} ${chalk.green(scheduler.job.name)} in ${execution.endTime.getTime() - execution.startTime.getTime()}ms`);
            ...
            //BREAKPOINT: LOGGING
            //consola.error(`${logSymbols.error} ${chalk.red('Job failed')} ${chalk.green(scheduler.job.name)}: ${error.message}`);
        } finally {
    ...
    function setupGracefulShutdown(): void {
        const shutdownHandler = async () => {
            //BREAKPOINT: LOGGING
            //consola.info(`${chalk.yellow('🛑 Enhanced scheduler shutting down...')}`);
            isShuttingDown = true;
            ...
            while (runningExecutions > 0 && (Date.now() - startTime) < maxWaitTime) {
                //BREAKPOINT: LOGGING
                //consola.info(`${chalk.yellow('⏳ Waiting for')} ${runningExecutions} running jobs to complete...`);
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('Enhanced scheduler shutdown complete')}`);
        };
    ...
    async function saveSchedulerState(): Promise<void> {
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.info(`${logSymbols.info} ${chalk.blue('Enhanced scheduler state saved')}`);
        } catch (error: any) {
            //BREAKPOINT: LOGGING
    ...
    async function loadSchedulerState(): Promise<void> {
        try {
            ...
            for (const savedJob of state.jobs) {
                ...
                if (existingScheduler) {
                    ...
                    if (definitionChanged) {
                        //BREAKPOINT: LOGGING
                        //consola.info(`${chalk.yellow('🔄 Job definition changed:')} ${chalk.green(savedJob.name)} - recalculating schedule`);
                    ...
                    if (config.cron.logCronDetails && existingScheduler.job.cronExpression) {
                        ...
                        //BREAKPOINT: LOGGING
                        //consola.info(`${chalk.blue('📅 Job restored:')} ${chalk.green(savedJob.name)} - ${result.description} - next: ${chalk.cyan(existingScheduler.job.nextRun.toLocaleString())}`);
                    }
                }
            }
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('Enhanced scheduler state loaded')} (saved at ${state.savedAt})`);
        } catch (error: any) {
            if (error.code === 'ENOENT') {
                //BREAKPOINT: LOGGING
                //consola.info(`${chalk.blue('No previous scheduler state found - starting fresh')}`);
            } else {
                //BREAKPOINT: LOGGING
    ...
    function validateAndAdjustNextRun(job: ScheduledJob, savedNextRun?: Date): Date {
        ...
        if (savedNextRun <= now) {
            //BREAKPOINT: LOGGING
            //consola.info(`${chalk.yellow('⏰ Saved next run is in the past for')} ${chalk.green(job.name)} - recalculating`);
        ...
        if (isRapidRestart) {
            //BREAKPOINT: LOGGING
            //consola.info(`${chalk.blue('⚡ Rapid restart detected for')} ${chalk.green(job.name)} - preserving saved timing`);
            return savedNextRun;
        }
        ...
        if (job.cronExpression || job.intervalMs) {
            ...
            if (timeDiff > config.restartBehavior.maxTimingDrift) {
                //BREAKPOINT: LOGGING
                //consola.info(`${chalk.yellow('🔧 Adjusting timing for')} ${chalk.green(job.name)} - drift of ${Math.round(timeDiff / 1000)}s detected`);
    ...
    function cleanupExecutionHistory(): void {
        if (executionHistory.length > config.cleanup.maxExecutionHistory) {
            ...
            //BREAKPOINT: LOGGING
            //consola.info(`${chalk.blue('🧹 Cleaned up')} ${removed.length} old execution records`);
        }
    }
    ...
    app.get("/executions", async function (request, reply) {
