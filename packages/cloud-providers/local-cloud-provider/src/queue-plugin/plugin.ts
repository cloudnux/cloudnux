...
export const queuesPlugin: FastifyPluginCallback<QueuePluginOptions> = async (
    ...
    async function processMessage(queueName: string, message: QueueMessage): Promise<void> {
        ...
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('Successfully processed message')} ${chalk.yellow(message.id)} in queue ${chalk.magenta(queueName)}`);
        } catch (error: any) {
            //BREAKPOINT: LOGGING
            //consola.error(`${logSymbols.error} ${chalk.red('Error processing message')} ${chalk.yellow(message.id)} in queue ${chalk.magenta(queueName)}: ${error.message}`);
            ...
            if (message.attempts >= config.maxRetries) {
                ...
                if (index >= 0) {
                    ...
                    if (config.retryBackoff) {
                        ...
                        //BREAKPOINT: LOGGING
                        //consola.info(`${chalk.blue('⏱️ Scheduling retry')} for message ${chalk.yellow(message.id)} in ${chalk.cyan(delayMs)}ms`);
                        setTimeout(() => {
    ...
    async function initializePersistence(): Promise<void> {
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('Queue persistence initialized:')} ${chalk.yellow(config.persistence.directory)}`);
        } catch (error: any) {
            //BREAKPOINT: LOGGING
            //consola.error(`${logSymbols.error} ${chalk.red('Failed to initialize queue persistence:')} ${chalk.yellow(error.message)}`);
        }
    }
    ...
    async function saveAllQueueStates(): Promise<void> {
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.success(`${logSymbols.success} ${chalk.green('All queue states saved to')} ${chalk.yellow(config.persistence.directory)}`);
        } catch (error: any) {
            //BREAKPOINT: LOGGING
            //consola.error(`${logSymbols.error} ${chalk.red('Failed to save all queue states:')} ${chalk.yellow(error.message)}`);
        }
    }
    ...
    async function saveQueueState(queueName: string): Promise<void> {
        try {
            ...
            //BREAKPOINT: LOGGING
            //consola.info(`${logSymbols.info} ${chalk.blue('Queue state saved:')} ${chalk.green(queueName)}`);
        } catch (error: any) {
            //BREAKPOINT: LOGGING
    ...
    app.get<{ Params: { queue: string } }>("/:queue/process-dlq", async function (request, reply) {
        ...
        //BREAKPOINT: LOGGING
        //consola.warn(`${logSymbols.warning} ${chalk.yellow('Moving')} ${chalk.red(dlqCount)} messages from DLQ to incoming queue for ${chalk.green(queueName)}`);
    ...
    app.get<{ Params: { queue: string } }>("/:queue/purge-dlg", async function (request, reply) {
        ...
        //BREAKPOINT: LOGGING
        //consola.warn(`${logSymbols.warning} ${chalk.yellow('Purging')} ${chalk.red(dlqCount)} messages from DLQ for ${chalk.green(queueName)}`);
